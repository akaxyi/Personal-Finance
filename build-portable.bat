...
REM Build a portable self-contained app-image (folder with EXE and bundled JRE)
REM Requirement: JDK 17+ (with jpackage)
...
REM Create portable app image under dist\APP_NAME\
jpackage ^
  --name "%APP_NAME%" ^
  --vendor "%VENDOR%" ^
  --app-version %VERSION% ^
...
if errorlevel 1 (
  echo jpackage failed to create the app image.
  pause
  exit /b 1
)

echo.
echo Portable app image created under %DIST%\%APP_NAME%\
echo To run, execute: %DIST%\%APP_NAME%\%APP_NAME%.exe
echo Optionally, zip the %DIST%\%APP_NAME% folder to distribute.
echo Done.
endlocal
